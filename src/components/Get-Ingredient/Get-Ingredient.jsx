...
import ingredientStyles from './Get-Ingredient.module.css'
import {CurrencyIcon, Counter} from '@ya.praktikum/react-developer-burger-ui-components'

export default function GetIngredient ({type, ingredients, handler}) {
        ...
        if (item.type === type){
            return (
                <article className={`pl-4 pr-4 ${ingredientStyles.ingredient}`} key={item._id} onClick={(event) => handler(event, item)}>
                    ...
                    <img src={item.image} className={ingredientStyles.image}/>
